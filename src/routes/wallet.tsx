...
import { Loader2, Wallet, ArrowDownCircle, ArrowUpCircle, CreditCard, RefreshCw, AlertCircle } from "lucide-react";
...
export default function WalletPage() {
  ...
  useEffect(() => {
    ...
    const existingMonnify = document.querySelector('script[src="https://sdk.monnify.com/plugin/monnify.js"]');
    if (!existingMonnify) {
      const m = document.createElement("script");
      m.src = "https://sdk.monnify.com/plugin/monnify.js";
      m.async = true;
...
function FundWallet({ user, wallet, onFunded }: { user: import("@supabase/supabase-js").User; wallet: WalletRow | null; onFunded: () => void }) {
  ...
  const handleNowPayments = async () => {
    ...
    try {
      const { error: intentErr } = await supabase.from("payment_intents").insert({
        user_id: user.id, provider: "monnify", reference: ref, amount: amt, currency: "NGN", status: "pending",
      });
      if (intentErr) { setNowLoading(false); return toast.error(`Failed to initialize payment: ${intentErr.message}`); }

      const MonnifySDK = (window as unknown as { MonnifySDK?: { initialize(o: Record<string, unknown>): void } }).MonnifySDK;
      if (!MonnifySDK) { setNowLoading(false); return toast.error("Monnify is still loading — please wait a moment and try again"); }
      ...
      MonnifySDK.initialize({
        ...
        currency: "NGN",
        reference: ref,
        customerFullName: user.user_metadata?.full_name ?? user.email ?? "Customer",
        ...
        onComplete: async (response: { paymentStatus: string; transactionReference: string }) => {
          if (response.paymentStatus === "PAID") {
            const tid = toast.loading("Verifying payment…");
            try {
              await verifyPaystackPayment({ reference: ref, userId: user.id });
              toast.dismiss(tid);
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <Button onClick={handleNowPayments} disabled={nowLoading || amt < 100} className="w-full bg-brand-orange hover:bg-brand-orange-hover text-white">
